...
- **AI Analysis**: Local Mac Studio LLM for summarization and Q&A
- **Real-time Dashboard**: React frontend with live data updates
...
- **Purpose**: Gmail API integration, attachment metadata storage, Drive upload, conversion job management
...
- **Image**: python:3.11-slim
- **Purpose**: Background conversion job processor, file format conversion, attachment processing pipeline
...
- **Python Dependencies** (23 packages):
  ...
  # Web Framework & HTTP
  fastapi==0.104.1, uvicorn[standard]==0.24.0, httpx==0.25.2
  requests==2.31.0, aiofiles==23.2.1, aiohttp==3.8.6
  ...
  pydantic==2.5.0, structlog==23.2.0, python-multipart==0.0.6, python-dotenv==1.0.0
  ```
- **Background Processing**: Polls conversion_jobs table every 10 seconds for pending work
- **Job Types**: attachment_conversion (PDFs, Word docs, images → markdown)
- **Processing Flow**: Download → Convert → Upload → Update database
- **Status**: ✅ Active with ConversionJobProcessor implementation
...
4. **Attachment Metadata Storage**: Store attachment info and create conversion jobs
5. **Background Processing**: Content Extractor polls for pending conversion jobs
6. **File Processing Pipeline**: Download → Convert → Upload to Drive → Update DB
7. **AI Categorization**: Assign to one of 6 categories
8. **Entity Extraction**: People, companies, technologies
9. **Database Storage**: Structured data with full-text search
10. **Status Update**: Mark as processed in Gmail
...
Email Processing → Attachment Metadata Storage → Conversion Job Creation
                                                           ↓
Content Extractor (Background) → Download from Gmail → File Processing → Google Drive Upload
                                                                              ↓
                                Database Update ← Markdown Content ← Format Conversion
```

#### **Processing Flow Details**
1. **Email Processor** (`port 3003`): Stores attachment metadata, creates conversion jobs
2. **Content Extractor** (`port 3005`): Background processor polls for pending jobs
3. **File Download**: Content extractor downloads file content via email processor API
4. **Format Conversion**: PDF→Markdown, Word→Markdown, Images→OCR text
5. **Drive Upload**: Processed files uploaded to Google Drive via email processor
6. **Database Update**: Attachment record updated with Drive URLs and markdown content
...
POST   /drive/upload                    # Upload file to Google Drive (internal)
...
GET    /jobs                           # Check conversion job status (Content Extractor)
POST   /jobs/process                   # Manually trigger job processing (Content Extractor)
...
### 📋 Phase 3: Mac Studio LLM Integration (FUTURE)
**Duration**: 1 week  
...
- AI analysis endpoints (summarization, categorization, Q&A)
- Interactive query system
- Performance optimization and caching

**Integration Points**:
- Mac Studio LLM endpoint configuration
- Unified context building pipeline
