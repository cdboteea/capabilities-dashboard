...
class FileConverter:
    ...
    async def convert_html(self, file_content: bytes, filename: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Add title
            title_content = f"# {Path(filename).stem}\n\n" + markdown_content
            ...
            logger.error(f"HTML conversion error for {filename}: {str(e)}")
            return {
                'content': f"# {Path(filename).stem}\n\n*Error processing HTML: {str(e)}*",
                'info': {'error': str(e)}
            }
    
    async def convert_text(self, file_content: bytes, filename: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Text conversion error for {filename}: {str(e)}")
            return {
